...
if ( ! class_exists( 'CMB2_Bootstrap_221', false ) ) {
	...
	class CMB2_Bootstrap_221 {
		...
		public function include_cmb() {
			...
			if ( ! defined( 'CMB2_DIR' ) ) {
				define( 'CMB2_DIR', trailingslashit( get_template_directory() . '/includes/metabox/cmb2' ) );
			}
		...
		public function l10ni18n() {
			...
			$loaded = load_plugin_textdomain( 'landy', false, '/languages/' );

			if ( ! $loaded ) {
				$loaded = load_muplugin_textdomain( 'landy', '/languages/' );
			}

			if ( ! $loaded ) {
				$loaded = load_theme_textdomain( 'landy', get_stylesheet_directory() . '/languages/' );
			}

			if ( ! $loaded ) {
				$locale = apply_filters( 'plugin_locale', get_locale(), 'landy' );
				$mofile = get_template_directory() . '/includes/framework/metabox/cmb2/languages/cmb2-' . $locale . '.mo';
				load_textdomain( 'landy', $mofile );
			}
